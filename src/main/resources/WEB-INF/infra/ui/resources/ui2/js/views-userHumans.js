		...
		resolve({
			...
			methods: {
				...
				create: function() {
					...
					console.log('UserHuman - Create', clonedForm)
					httpPost('/api/userHuman/createByEmail/' + c.form.userEmail, null, function(data) {
						c.formResult = data
						if (data.success) {
							jQuery('#createModal .btn-secondary').click()
							successShow(c.$t('prompt.create.success', [c.form.userEmail]))
							c.refresh()
						}
					})
				},

				createShow: function(userHuman) {
					var c = this
					c.formResult = {}
					c.form = {}
